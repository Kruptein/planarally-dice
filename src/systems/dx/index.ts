import { rollParts, rollString } from "../../core";
import { type DiceSystem, type Roller, type RollResult, Status, type WithStatus } from "../../core/types";
import {
    type DieSegment,
    type DxSegment,
    DxSegmentType,
...
export class DxSystem implements DiceSystem<DxSegment> {
    roller: Roller<DxSegment>;
    name = "Dx";

    constructor(roller?: Roller<DxSegment>) {
        this.roller = roller ?? new DxRoller();
    }

    async rollParts(parts: DxSegment[]) {
        return await rollParts(
            parts.map((p) => ({ ...p, status: Status.Unknown })) as WithStatus<DxSegment, Status>[],
            this,
        );
    }

    async rollString(input: string) {
        return await rollString(input, this);
    }

    parse(input: string): WithDxStatus<DxSegment, Status.PendingEvaluation | Status.PendingRoll | Status.Resolved>[] {
        ...
        for (const part of input.matchAll(regex)) {
            ...
            if (part.groups?.fixed !== undefined) {
                ...
                data.push({
                    ...
                    selector,
                    selectorValue:
                        part.groups.selval !== undefined ? Number.parseInt(part.groups.selval, 10) : undefined,
    ...
    evaluate(part: WithDxStatus<DxSegment, Status.PendingEvaluation>): WithDxStatus<DieSegment, Status.Resolved> {
    ...
    collect(parts: WithDxStatus<DxSegment, Status.Resolved>[]): RollResult<DxSegment> {
        let total = 0;
